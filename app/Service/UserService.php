...
class UserService
{
  ...
  public function findDetailProfile(
    string $NamaPengguna
  ): UserProfileDetailResponse {
    try {
      Database::beginTransaction();

      $user = $this->userRepository->findByUsername($NamaPengguna);
      if ($user == null) {
      ...
      $response = new UserProfileDetailResponse();
      $response->user = $user;
      return $response;
    } catch (\Exception $exception) {
      Database::rollbackTransaction();
  ...
  private function validateUserProfileUpdateRequest(
    UserProfileUpdateRequest $request
  ) {
    if (
      $request->id == null ||
  ...
  private function validateUserPasswordUpdateRequest(
    UserPasswordUpdateRequest $request
  ) {
    if (
      $request->id == null ||
      ...
      throw new ValidationException(
        'Id, Old Password, New Password can not blank'
      );
    }
  }
